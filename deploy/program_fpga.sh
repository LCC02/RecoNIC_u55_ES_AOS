...
usage_func() {
  ...
  -p, --prog_file    FPGA programming file in \"mcs\" format
...
if [ -e "/sys/bus/pci/devices/${pcie_bdf_num}/driver" ]; then
  ...
  if [[ "$drv_name" != "qdma-pf" && "$drv_name" != "qdma-vf" && "$drv_name" != "xdma" && "$drv_name" != "onic" ]]; then
    echo "Error: Unrecognized driver: $drv_name"
    exit 1
  fi
